...
export const TicketForm: React.FC<TicketFormProps> = ({ onSuccess, onCancel, defaultCategory }) => {
    ...
    const categories = [
        { id: 'ORDERS', label: t.dashboard.support.categories.orders },
        { id: 'RETURNS', label: t.dashboard.support.categories.returns },
        { id: 'PAYMENT', label: t.dashboard.support.categories.payment },
        { id: 'TECHNICAL', label: t.dashboard.support.categories.technical },
        { id: 'OTHER', label: t.dashboard.support.categories.other }
    ];
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await createSupportChat(
                finalSubject,
                description,
                undefined, // orderId (can be linked later)
                attachment?.url,
                attachment?.type,
                attachment?.name,
                priority
            ...
            console.error('Failed to submit customer ticket:', err);
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-2xl font-bold text-white">
                            {t.dashboard.support.newTicketTitle}
                        </h3>
                        <p className="text-white/40 text-sm mt-0.5">
                            {language === 'ar' ? 'نظام دعم العملاء - استجابة فورية' : 'Customer Support System - Instant Response'}
                        </p>
                    </div>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                    ...
                    <input
                        ...
                        placeholder={t.dashboard.support.form.summaryPlaceholder}
                        required
                    />
                </div>
                ...
                <div>
                    <label className="block text-sm font-semibold text-white/70 mb-2.5 ml-1">
                        {language === 'ar' ? 'تصنيف المشكلة' : 'Issue Category'}
                    </label>
                    <div className="relative">
                        ...
                        <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none text-white/20 group-hover:text-gold-500">
                             {/* Custom arrow if needed, but select default is fine for now */}
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <label className="block text-sm font-semibold text-white/70 mb-2.5 ml-1">
                        {t.dashboard.support.form.priority}
                    </label>
                    <div className="flex p-1.5 bg-[#12110F] border border-white/5 rounded-2xl gap-2">
                        {['LOW', 'MEDIUM', 'HIGH'].map(p => (
                            ...
                            >
                                {t.dashboard.support.form[p.toLowerCase() as keyof typeof t.dashboard.support.form]}
                                {priority === p && p === 'HIGH' && (
                ...
                <div className="md:col-span-2">
                    ...
                    <textarea
                        ...
                        placeholder={t.dashboard.support.form.descPlaceholder}
                        required
                    />
                </div>
                ...
                <div className="md:col-span-2">
                    <label className="block text-sm font-semibold text-white/70 mb-2.5 ml-1">
                        {language === 'ar' ? 'المرفقات والصور (اختياري)' : 'Attachments & Photos (Optional)'}
                    </label>
                    <input
                        type="file"
                        ...
                        onChange={async (e) => {
                            ...
                            if (file) {
                                try {
                                    setIsSubmitting(true);
                                    const url = await storageApi.upload(file, 'support-files', 'customer-tickets');
                    ...
                    >
                        {attachment ? (
                            ...
                            <>
                                ...
                                <p className="text-white font-bold text-lg mb-2">{t.dashboard.support.form.upload}</p>
                        ...
                        {isSubmitting && !attachment && (
                            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center gap-4 z-10">
                                ...
                                <p className="text-gold-500 font-bold">{t.dashboard.support.form.submitting}</p>
                            </div>
                        )}
                    </label>
                </div>
                ...
                >
                    {isSubmitting ? (
                        <>
                            ...
                            <span>{t.dashboard.support.form.submitting}</span>
                        </>
                    ) : (
                        <>
